...
const addLog = async (req,res) => {
  const {admin_id , activity:text} = req.body;
  try {
    ...
    if(isFound.length > 0) {
      ...
      await AdminLog.updateOne({admin_id} , {activity : dActivity});
    }else
      await AdminLog.create({admin_id , activity:[{text}]});
    res.status(200).send();
