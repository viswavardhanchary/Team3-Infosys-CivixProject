...
export const Petitions = () => {
  ...
  <div className="flex items-center justify-between">
      ...
      {["All Petitions", "My Petitions", "Signed by Me"].map((filter) => (
        ((isAdmin && filter !== "My Petitions") || !isAdmin) && (
          <button
            ...
            className={`px-4 py-2 rounded-md ${buttonsColor[filter === "All Petitions" ? "All" : filter] ? "bg-[#003f7f]" : "bg-[#0055A4]"} hover:bg-[#003366] text-white font-semibold transition cursor-pointer`}
          >
            {filter}
          </button>
        )
    ...
    <div className="flex flex-wrap gap-2 mt-2 justify-end text-sm md:text-lg">
      <select className="p-2 rounded-md border border-[#0055A4] bg-[#1e293b] text-white outline-none cursor-pointer"
  ...
  <div className="flex flex-col gap-4">
    {petitions.length === 0 ? (
      <div className="flex flex-col items-center justify-center gap-3 p-6 bg-[#1e293b] rounded-md shadow-md border border-[#333333] text-center">
        ...
        <button className="px-4 py-2 rounded-md bg-[#0055A4] hover:bg-[#003366] text-white font-semibold transition">
          Clear Filters
        </button>
      </div>
    ) : (
